...
function normalizeAction(value = "") {
  ...
  if (["on", "visible", "user", "activar", "encender", "prender"].includes(raw)) return "user";
  if (["owner", "debug", "full"].includes(raw)) return "owner";
  if (["off", "disable", "apagar", "desactivar"].includes(raw)) return "off";
  return "";
}

export default {
  name: "antierror",
  command: ["antierror", "errorvisible", "erroresvisibles", "antierrorvisible"],
  category: "sistema",
      ...
      return sock.sendMessage(
        from,
        {
          text:
            ...
            `• ${prefix}antierror on (activar)\n` +
            `• ${prefix}antierror owner\n` +
            `• ${prefix}antierror off (desactivar)`,
          ...global.channelInfo,
        },
        { quoted: msg }
      );
